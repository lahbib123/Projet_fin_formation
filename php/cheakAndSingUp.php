...
if (strlen($fullName) < 6 || !preg_match("/^[aA-zZ]+[\s_][aA-zZ]+$/", $fullName)) {
    ...
    $retry = 1;
    $error = 'acceptTerms';
    $alert = "Terms And Conditions_You cannot register on the site without agreeing to the terms_white_red";
} else {
    $req = "INSERT INTO `users_accounts` (`fullName`,`email`,`password`,`phone`,`DOB`,`gender`,`city`,`address`)
        VALUES ('$fullName','$email','$password','$phone','$DOB','$gender','$city','$address')";

    $singup = mysqli_query($connect, $req);
    $req2 = "INSERT INTO `all_accounts` (`account`, `user_admin`) VALUES ('$email', 'user')";
    mysqli_query($connect, $req2);
    if ($singup) {
        header('location:login.php?alert=Account created_Your account has been created successfully, you can now log in_white_green');
    }
}

if ($retry) {
    header('location:' . $_SERVER['PHP_SELF'] . "?retry=$retry&error=$error&alert=$alert&fullName=$fullName&email=$email&password=$password&phone=$phone&city=$city&address=$address&day=$day&month=$month&year=$year&gender=$gender");
}
